...
MAX_TIME = config_dic["MAX_TIME"] # 设置最大迭代次数(如果方案数量筛选到只剩一个就提前结束了)
...
def generate_path(nodes,index):
    ...
    # while len(nodes) > 0:
    #     print("当前节点为：", node[index])
    #     path.extend(index)
    #     nodes.remove(node[index])  # 从节点列表中移除已规划的节点
    # 
    #     if len(nodes) == 0:  # 所有节点都规划了
    #         break
    #     # 获取当前节点到其他节点的距离
    #     distances_to_other = distance_matrix[index]
    # 
    #     # 获取未到达节点的概率列表，距离越近概率越大
    #     prob_to_other = weights[nodes.index(current_node):nodes.index(current_node) + len(nodes)]
    #     # 根据距离加权随机选择下一个节点
    #     next_node = np.random.choice(nodes, size=1, p=prob_to_other)[0]
    #     print("下一步节点为：", next_node)
...
def init_population(n_chromosomes, n_cities):
    ...
    insert_positions = sorted(random.sample(range(1, n_cities - 1), ROBOT_NUMBER-1))
